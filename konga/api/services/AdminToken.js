...
module.exports.issue = function issue(payload) {
    ...
    return jwt.sign(
        ...
        process.env.TOKEN_SECRET || "adminsecret" // Secret string which will be used to sign the token
    );
};
...
module.exports.verify = function verify(request ,token, next) {
    ...
    return jwt.verify(
        ...
        process.env.TOKEN_SECRET || "adminsecret", // The secret we used to sign it.
...
module.exports.getToken = function getToken(request, next, throwError) {
    ...
    if (request.cookies['Admin-token']){
        token = request.cookies['Admin-token'];
    }
